...
SPECIFIC_RULES = [
    ...
    ("코디마스터",          r"코디마스터"),
    # 아이스크림에듀·아이스크림미디어의 학습 제품. '아이스크림'만으로는 먹는 아이스크림(간식)과
    # 아이스크림몰에서 산 종이 교재('아이스크림 한걸음 수학' 등)까지 딸려 온다.
    # 브랜드가 분명히 적힌 표기만 인정하고, 단독 표기는 아래 CTX_ICECREAM에서 소프트웨어 문맥을 본다.
    ("아이스크림",          r"아이스크림 ?에듀|아이스크림 ?미디어|아이스크림 ?홈런|"
                            r"아이스크림 ?[Ss](?![가-힣])|i-?Scream ?(?:edu|media|home|s\b)|"
                            r"아이스크림(?=[^가-힣]{0,6}(?:구독|라이선스|라이센스|이용권|플랫폼|"
                            r"코스웨어|디지털|온라인|소프트웨어|콘텐츠|계정))"),
    ("젭(ZEP)",            r"젭|\bZEP\b"),
    ("매쓰플랫",            r"매쓰플랫"),
    ("스쿨플랫",            r"스쿨플랫"),
    ("퀴즈앤",             r"퀴즈앤|QuizN"),
