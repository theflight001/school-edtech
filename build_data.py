...
SPECIFIC_RULES = [
    ...
    ("Zoom",                r"\bZoom\b(?!.*(?:레코더|녹음|믹서|렌즈|마이크|H\d))|줌\s?(?:프로|라이선스|라이센스|미팅|회의|계정|구독)|줌\s?X\s?체더스"),
    ("미리캔버스",          r"미리캔버스"),
    ("Padlet",             r"Padlet|패들렛"),
    ("하이러닝",            r"하이러닝|Hi-?Learning"),
    # 바당은 제주말로 바다다 — 축제·운동회·해양활동 이름에 흔하다(20건).
    ("바당",               r"바당(?!.*(?:축제|운동회|한마당|체험|캠프|공기순환))|BADANG"),
    ...
    ("한컴오피스",          r"한컴 ?오피스|한글과 ?컴퓨터|아래아 ?한글|\bHWP\b|한/글|"
                            r"한글\s?20[0-9]{2}(?!.*(?:수험서|문제집|교재|기출|자격|필기|실기))|"
    ...
    ("엘리스",                 r"엘리스(?!뮤직)(?!.*(?:악기|소금|기타|피크|단소))|\belice\b"),
    ("니어팟",             r"니어팟|Nearpod"),
    # 마시는 밀크티가 제품 밀크T로 잡혔다(70건). 천재교육 제품은 영문 T를 쓴다.
    ("밀크T",              r"밀크\s?T(?![가-힣])|밀크티\s*(?:구독|이용권|학습|라이선스|라이센스)"),
    ...
    ("Suno",                r"\bSuno\b|수노\s?(?:AI|프로|Pro)|(?<![가-힣])수노(?![가-힣])"),
    ...
    ("Gamma",               r"\bGamma ?(?:Pro|AI)?\b|감마\s?(?:프로|AI)|(?<![가-힣])감마(?![가-힣선])"),
    ...
    ("네프론",               r"네프론(?!\s*(?:원리|모형|만들기|구조|실험|관찰))|Nephron"),
