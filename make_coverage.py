...
def main():
    ...
    def why(src, m, c):
        ...
        if first and m < first:
            return f"수집 자료가 그 시기부터 시작함(원천 보유 여부 미확인 · 가장 이른 자료 {first//100}.{first%100:02d})"
        if last and m > last:
            return "아직 공개 전"
        ...
        return "수집 자료에 그달 계약이 없음(추정 — 원천 미확인)"
    ...
    with open(OUT, "w", encoding="utf-8-sig", newline="") as f:
        ...
        w.writerow(["# 빈 칸에는 왜 비었는지 아래 '비고'에 적었습니다. 사유는 우리 수집 파일의 모양으로 추정한 것이며, 원천 사이트에서 그달을 직접 확인한 것이 아닙니다."])
        w.writerow([])
