# 인천교육청 계약공개 수집분 정제 — 학교 매칭 + 에듀테크 판정
# 사용: python3 refine_ice.py
# 입력: ice_candidates.csv   출력: ice_refined.csv (refined_full.csv와 같은 스키마)
...
def match_school(name):
    """인천 소속이므로 인천 학교 중에서 찾는다"""
    nm = ALIAS.get(name, name)
    cands = [c for c in master_by_name.get(nm, []) if c["sido"].startswith("인천")]
    if len(cands) == 1:
        return nm, cands[0]
    # '인천OO초등학교' ↔ 'OO초등학교' 표기 차이 보정
    alt = nm[2:] if nm.startswith("인천") else "인천" + nm
    cands = [c for c in master_by_name.get(alt, []) if c["sido"].startswith("인천")]
    if len(cands) == 1:
        return alt, cands[0]
    return nm, None

def main():
    rows = list(csv.DictReader(open("ice_candidates.csv", encoding="utf-8-sig")))
    ...
    for r in rows:
        ...
        out.append({
            "계약번호": f"ICE-{key}", "구분": r.get("구분") or "물품", "계약명": name,
            "계약일": r.get("계약일", ""), "금액": r.get("계약금액", ""),
            "수요기관": f"인천광역시교육청 {school_raw}", "학교명": school,
            "업체명": r.get("계약상대자", ""),
            "학교코드": m["code"] if m else "", "급별": m["level"] if m else "",
            "시도": m["sido"] if m else "인천광역시", "상세URL": "",
        })
    with open("ice_refined.csv", "w", encoding="utf-8-sig", newline="") as f:
        w = csv.DictWriter(f, fieldnames=FIELDS)
        w.writeheader()
        w.writerows(out)
    print(f"수집 {len(rows)}건 → 에듀테크 확정 {len(out)}건 (학교 매칭 {matched}건)")
    print("제외 사유:", dict(drop))
    print(f"학교 {len({r['학교명'] for r in out})}개교 → ice_refined.csv")
