# 부산교육청 계약공개 수집분 정제 — 학교 매칭 + 에듀테크 판정
# 사용: python3 refine_pen.py
# 입력: pen_candidates.csv   출력: pen_refined.csv (refined_full.csv와 같은 스키마)
...
def match_school(name):
    """부산 소속이므로 부산 학교 중에서 찾는다"""
    nm = ALIAS.get(name, name)
    cands = [c for c in master_by_name.get(nm, []) if c["sido"].startswith("부산")]
    if len(cands) == 1:
        return nm, cands[0]
    # '부산OO초등학교' ↔ 'OO초등학교' 표기 차이 보정
    alt = nm[2:] if nm.startswith("부산") else "부산" + nm
    cands = [c for c in master_by_name.get(alt, []) if c["sido"].startswith("부산")]
    if len(cands) == 1:
        return alt, cands[0]
    return nm, None

def main():
    rows = list(csv.DictReader(open("pen_candidates.csv", encoding="utf-8-sig")))
    ...
    for r in rows:
        ...
        out.append({
            "계약번호": f"PEN-{key}", "구분": "물품", "계약명": name,
            "계약일": r.get("계약일", ""), "금액": r.get("계약금액", ""),
            "수요기관": f"부산광역시교육청 {school_raw}", "학교명": school,
            "업체명": r.get("계약상대자", ""),
            "학교코드": m["code"] if m else "", "급별": m["level"] if m else "",
            "시도": m["sido"] if m else "부산광역시", "상세URL": "",
        })
    with open("pen_refined.csv", "w", encoding="utf-8-sig", newline="") as f:
        w = csv.DictWriter(f, fieldnames=FIELDS)
        w.writeheader()
        w.writerows(out)
    print(f"수집 {len(rows)}건 → 에듀테크 확정 {len(out)}건 (학교 매칭 {matched}건)")
    print("제외 사유:", dict(drop.most_common()))
    print(f"학교 {len({r['학교명'] for r in out})}개교 → pen_refined.csv")
