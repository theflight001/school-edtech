...
# ── 0-1. 전남을 다른 기기에서 받는 동안은 여기서 건드리지 않는다 (2026-09-15)
#      이 IP는 전남이 막았고, 노트북이 다른 IP로 이어받는 중이다. 표지 파일이 있는 동안 전남은 건너뛰고,
#      전남이 빠진 판은 배포하지 않는다(수집 빠진 채 게시 금지). 노트북 결과를 되돌린 뒤 표지 파일을 지운다.
if [ -e .전남_이관중 ]; then
  export EDTECH_SKIP="전남 ${EDTECH_SKIP:-}"
  FAILED+=("전남(노트북에서 수집 중 — .전남_이관중)")
  echo "▷ 전남 — 노트북에서 받는 중이라 건너뛴다(.전남_이관중). 이번 판은 배포하지 않는다"
fi
